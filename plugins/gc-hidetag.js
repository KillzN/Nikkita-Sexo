//Código desarollado por Semi, https://wa.me/51947248937)
let handler = async (m, { conn, text, participants }) => {
  ...
  if (!m.quoted && type) {
    if ((mime === 'imageMessage')) {
      ...
      conn.sendMessage(m.chat, { text: text ? text : "Pᴏʀɴʜᴜʙ: @BʏNikkita", mentions: participants.map(u => conn.decodeJid(u.id)) }, { quoted: m })
    }
  } else if (m.quoted) {
